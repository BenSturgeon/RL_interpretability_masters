...
\subsection{The Effectiveness of SAEs for Interpretability}
% From Paper 1 Discussion
The results suggest that while Sparse Autoencoders (SAEs) do aid in disentangling features within RL agents, there are nuances to their utility. In some respects, the base model, despite its polysemanticity, may offer more straightforward interpretations for certain phenomena due to clearer delineations between primary tracking channels and others before SAE processing. However, the emergence of unique, more disentangled channels for specific entities (like keys) within the SAE is a notable advantage for targeted analysis and intervention. The variability in results depending on SAE training duration also highlights that stability in SAE training is an important factor to consider in interpretability work.
...
Furthermore, the observed specialization of roles within SAE channels (e.g., steering-dominant vs. navigation-supportive channels) hints at a distributed yet specialized computational architecture emerging from unsupervised SAE training. This could inform theories on how complex behaviors are decomposed and represented within neural networks.
...
\end{itemize}
The techniques themselves, such as targeted activation steering or the analysis of SAE features, could become part of a standard toolkit for RL practitioners seeking to verify and validate their models beyond simple performance metrics.
