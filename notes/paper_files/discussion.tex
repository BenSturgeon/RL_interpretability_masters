...
\begin{table}[h]
\centering
\caption{Hypothesis outcomes with key supporting evidence.}
...
While some of our initial hypotheses proved correct, we were surprised by the discovery of the activation magnitude mechanism as a means of encoding goals. We also found that SAEs did not work as expected in this case, but as a result were able to validate some of our new results. We were also able to better characterise some of our initial ideas about how the network might work.

\subsection{Research Questions}

\begin{table}[h]
\centering
\caption{Research questions and their answers based on experimental evidence.}
...
\midrule
RQ1 & How are goals encoded? & Via activation magnitude multiplexing across many channels (Section~\ref{sec:parallel_rollouts},~\ref{sec:offset_steering}) \\
...
Our findings in Section~\ref{sec:parallel_rollouts} and~\ref{sec:offset_steering} show that rather than requiring separate channels for each entity type, the model has learned to use activation magnitude as an additional dimension for encoding information. 
...
The activation jumps observed regardless of collection order, occurring even when the green key is collected first (12\% of cases), suggest that these jumps are tied to collection events rather than to which specific entity is collected. This indicates separate mechanisms for tracking collection progress and for detecting specific entities, as shown by the conv1a probe results presented in Appendix~\ref{app:channel_probe_results}.

One challenge in making this discovery was that many of the tools typically used in mechanistic interpretability were not designed to expose activation magnitude as an encoding dimension. SAEs and probes successfully identified which channels responded to which entities, but did not reveal that the magnitude of those responses carried additional information about the current goal stage. What initially appeared to be polysemanticity was instead a structured strategy for encoding goal state through activation levels.
...
This variety of solutions suggests that while amplitude-based multiplexing appears consistently, the specific channels used for this encoding are not fixed but drift across checkpoints. The continuous drift between encoding schemes post-convergence indicates the network exists on a `valley floor' of equally viable solutions, constantly reorganizing its internal representations while preserving behavioural performance. 
