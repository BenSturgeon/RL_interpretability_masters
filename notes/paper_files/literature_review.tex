...
Dictionary learning~\citep{olshausen_1996_emergence} provides a principled approach to resolving superposition (Section~\ref{subsec:superposition}). The goal is to discover an overcomplete dictionary $D \in \mathbb{R}^{d \times k}$ (where $k > d$) whose columns serve as basis vectors that can represent input data $x \in \mathbb{R}^d$ through sparse linear combinations:
