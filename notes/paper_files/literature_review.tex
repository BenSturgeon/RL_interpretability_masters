...
Value functions are foundational to many training algorithms. Two fundamental approaches exist for estimating them: Monte Carlo methods, which use complete episode returns, and Temporal Difference (TD) learning, which bootstraps from current estimates. These algorithms alternate between sampling data from the environment and optimising objectives via stochastic gradient ascent. 
...
Policy gradient methods arise from the natural challenges introduced by trying to learn optimal policies within continuous action spaces and with non-deterministic policies. Rather than learning state values using these to indirectly develop a policy, policy gradients directly optimise parameterised policy $\pi_\theta(a|s)$ by gradient ascent on expected returns.
...
In another example, researchers used MI techniques to extract a novel chess concept from AlphaZero and successfully taught it to grandmasters, who reported genuinely learning something new about the game.~\citep{cunningham2025cheap} demonstrated that linear probes trained on intermediate activations can detect jailbreak attempts and dangerous content (including bioweapon-related queries) in real-time, achieving detection performance comparable to dedicated classifiers with orders of magnitude more parameters.
...
\end{equation}
A neuron $i$ is polysemantic when the $i$-th row of the mixing matrix $M$ has significant weight on multiple features (row sparsity $\lVert M_{i,:}\rVert_{0}>1$).  The toy model of \citep{elhage2022superposition} shows that packing many sparse features ($k$ large, $s$ sparse) into a narrow layer of width $d$ forces an unavoidable trade-off:
...
Techniques such as those developed by~\citep{zeiler_2013_visualizing} represented the first work on interpreting the operations of deep neural networks. They employ a deconv-net, a model that maps feature activations from a deep layer back to the input pixel space by approximately reversing the network's pooling and convolutional operations. By reconstructing the maximally activating zones in pooling layers and being chained through subsequent CNN layers the reconstruction of complex visual representations of deep layers within the network can be achieved. This is normally very challenging due to the significant compression that is performed on the deep layers in the CNN. By applying this deconvolution the features appearing deeper in the network can be expanded back up to human interpretable scales.
...
An instructive example comes from Othello-GPT~\citep{li_2024_emergent}, where a model trained to predict legal moves was shown to encode board state internally. Initially, 2-layer MLP probes achieved 98.7\% accuracy extracting piece positions while linear probes achieved only 20\%. However,~\citep{nanda_2023_emergent} showed that linear probes could match this performance when labels were changed from `black/white' to `mine/yours', revealing that the MLP probes had been performing additional computation rather than simply reading off representations.
...
Interpretability in RL presents unique challenges. The agent's interaction with an environment means that specific outputs of the model may be semantically distant from the contents of a given activation. Furthermore, we can only infer the meaning of an activation within the model; we cannot directly glean it as we could a word extracted from the embedding space of a language model.
...
More concrete technical research began to develop following the release of~\citep{Amodei_2016_AISafety}, which identifies concrete problems in AI safety, which are discussed more in the next section.
...
Several areas in RL interpretability remain underexplored:
