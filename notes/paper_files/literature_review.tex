...
However, visual environments such as Atari game environments have state spaces that would be computationally infeasible to solve with a lookup table (64×64×3 = 12,288 dimensional input with \(256^{12,288}\) possible states for 8-bit RGB images)\cite{mnih_2013_playing}. This necessitates function approximation, which does not provide convergence guarantees, and causes the learned representations in our models to become opaque. This opacity is a further motivator for the need for mechanistic interpretability on models of the kind studied in our research.
...
Among the various architectures developed for visual RL, the IMPALA encoder has emerged as a particularly effective baseline for challenging environments like Procgen. Originally introduced by\cite{pmlr-v80-espeholt18a} as a scalable RL framework, its convolutional encoder has since become a primary baseline for Procgen and Atari agents\cite{pmlr-v80-espeholt18a}. The encoder consists of three ``IMPALA blocks'', each containing:
\begin{itemize}
...
Classical algorithms such as K-SVD~\cite{aharon_2006_ksvd} and FISTA~\cite{doi:10.1137/080716542} solve this optimization problem through iterative sparse coding and dictionary updates. SAEs provide a neural network approach to this same objective.
...
\]
Following~\citep{gorton_2024_missing}, the extent to which concept $C$ influences the logit $f_{k}(x)$ for class $k$ is measured by the directional derivative
\[
