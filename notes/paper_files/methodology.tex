...
\subsection{Feature Visualization}
    ...
    \subsubsection{Determining the role of activation ranges in policy preferences}
    To test whether different activation ranges could affect the network's choice of which entity to target, we conducted a controlled experiment. We measured activation ranges associated with specific entities from the parallel rollout experiments in~\ref{subsubsec:parallel_rollout}, then applied the offset between a default target entity (e.g. the blue key) and another entity (e.g. red key) to all activation values in a given channel. We calculated offsets on a per-seed basis, as different seeds showed significantly different activation ranges, likely due to variations in background color and spatial layouts.

    We performed rollouts capturing mean activations across 400 unique maze seeds while swapping entities to generate observations of each entity type (blue key, green key, red key, gem). This yielded mean activation values for each channel in conv4a and conv3a when the agent was targeting each entity.
    
    After collecting these entity-specific activation patterns, we applied the offsets to test behavioral effects. We primarily used the blue key as baseline due to its natural position in the preference hierarchy. To steer toward a target entity, we added the per-channel, per-seed difference between blue key activations and the target entity's activations during new rollouts.
    
    To describe this formally: for a given maze seed $s$, channel $c$ in layer $l$, and entity $e$, the mean activation during entity-targeting rollouts is:

    \begin{equation}
    \mu_s^{c,l}(e) = \frac{1}{T} \sum_{t=1}^{T} h_t^{c,l}(e, s)
    \end{equation}

    where $h_t^{c,l}(e, s)$ is the channel activation at timestep $t$ and $T$ is the rollout length.

    The per-seed, per-channel offset between entities $e_1$ and $e_2$ is:

    \begin{equation}
    \Delta_s^{c,l}(e_1 \to e_2) = \mu_s^{c,l}(e_2) - \mu_s^{c,l}(e_1)
    \end{equation}

    During offset transfer experiments, we modify activations as:

    \begin{equation}
    \tilde{h}_t^{c,l} = h_t^{c,l}(s) + \Delta_s^{c,l}(e_1 \to e_2)
    \end{equation}

    where $h_t^{c,l}(s)$ is the natural activation at timestep $t$ in seed $s$.
...
\section{Expanded ablation experiments}
...
\textbf{Ablation 2 (Navigation channels):} We completely ablate the top 10 channels that had the highest collection rates for a given entity in our independent navigation study, setting these channels to zero while leaving the rest unchanged.

\textbf{Ablation 3 (Combined):} We ablate both the navigations that had the best collection rates for a given entity channels and the intervention spans for a given entity simultaneously.
...
\textbf{Ablation 5 (Random):} We randomly select 10 channels (equal to the number of navigation channels ablated) and completely ablate them as a baseline.
...
To investigate where the network encodes information about which entity to target next, we trained binary linear probes to predict the next target entity from layer activations.

For determining the next entity, we gathered a large number of observations from many unmodified maze rollouts, capturing activations every 7 steps to ensure sufficient diversity was achieved. For each of seven entity types (blue key, green key, red key, gem, blue lock, green lock, red lock), we gathered 2000 observations where that entity was the agent's next target.

For each entity type, we trained a binary classifier to predict whether that entity was the next target. The training set consisted of 4000 samples: 2000 positive examples (target entity) and 2000 negative examples (randomly sampled from other entity types). We used a 80/20 train-test split.

We trained two types of probes:
\begin{itemize}
    \item \textbf{Per-channel probes}: For each channel in layers conv2a-conv4a, 
    we flattened the spatial activations and trained a linear classifier
    \item \textbf{Per-layer probes}: For full layers (conv2a, conv3a, conv4a, 
    fc1, fc2, fc3), we flattened all activations across all channels/neurons 
    and trained a single linear classifier for each layer
\end{itemize}

All probes used logistic regression with L2 regularization, trained until 
convergence using the Adam optimizer.

\subsection{Direction probes}\label{subsec:directional_probes}
To establish which parts of the network were more involved with navigation, we trained additional binary linear probes designed to detect which direction the agent would need to go in order to reach the next target entity. This is a complex task because often the direction needed to travel in was significantly different than the position of the entity relative to the player.

Building the training dataset involved using Djikstra's algorithm to calculate the most efficient route to the next entity and using the grid state to calculate which path would need to be taken, and produce the direction that would need to be taken in the most efficient route. For each cardinal direction we gathered 2000 observations, excluding examples where the agent was directly next to the target entity, or where the agent was too far from the edge of an actual grid square in the maze, ensuring that there was no ambiguity in the direction that needed to be taken.

We used the same approach as in~\ref{subsec:next_entity_probes} for our sample distribution and train-test split.
