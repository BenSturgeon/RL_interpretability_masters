...
\subsection{Activation Visualization}
    ...
    A useful initial approach involved creating direct visualisations of the convolutional layers of the model by taking a given layer and applying a colour spectrum according to the strength of the activations within that region of the grid. This approach, following~\citep{mini_2023_understanding}, provided a direct view of which spatial regions triggered strong positive or negative responses in different channels. Figure~\ref{fig:filter_visualisations} shows an example of this visualisation technique applied to multiple layers.
    ...
    \begin{figure}[ht]
        ...
        \caption[Visualization of maze and neural network activations across layers]{Visualization of the maze and neural network activations in different layers. Top row (left to right):
        observation input, conv1a channel 10, conv2a channel 10, and activation colorbar.
    ...
    SAEs are a type of unsupervised learning technique that can be used to break down the most critical functions of a model into a more interpretable set of sparse latents, often by training an overcomplete basis on the original activations of a given layer of the network~\citep{bricken2023monosemanticity}. In this study, we apply this approach to the convolutional neural network (CNN) layers to disentangle model functionality. The training of SAEs directly addresses RQ3 by applying pressure to split polysemantic neurons into interpretable features, if those features are indeed merely polysemantic. Our methodology for adapting SAEs to CNNs builds upon recent work by~\citep{gorton_2024_missing}.

    SAEs were trained on the CNN layers of the compressed IMPALA model. Each model followed the 1×1-conv recipe of~\citep{gorton_2024_missing} with an L1 warm-up schedule. Training ran for 6\,M steps, during which time variance-explained rapidly exceeded 99\% and policy reward recovered to within 99\% of baseline. A compressed variant, with conv4a reduced to 20 channels, was also trained to test robustness under capacity constraints. Table~\ref{tab:sae-params-methodology} lists the expansion factors and L1 coefficients for all layers. 5 SAEs were trained for each layer, as a means of verifying the robustness of our results.
...
Hyperparameter selection followed conventions established in prior work on CNN SAEs~\citep{gorton_2024_missing}, who used expansion factors of 2$\times$ for early convolutional layers and 4$\times$ for middle layers when applying SAEs to InceptionV1. We adopt similar values: a 2$\times$ expansion for conv1a (the earliest layer, which learns low-level features requiring less disentanglement) and 4$\times$ for deeper layers. This modest expansion avoids excessive feature splitting while still providing sufficient capacity to decompose polysemantic neurons. L1 coefficients were tuned per-layer to balance reconstruction fidelity against sparsity: early layers used smaller coefficients (1e-4) to preserve fine-grained spatial information, while deeper layers tolerated larger coefficients (5e-3) as their representations are more abstract and naturally sparser.
    ...
    To establish which channels were critical to the overall functioning of the network, we performed a similar experiment to the one described in Section~\ref{subsec:discovering_independent_navigation} but instead of ablating all channels but one, we ablated only one channel to observe the impact on the collection rates of different entities. We repeated this for every channel in conv4a for n=400 rollouts in the fork maze shown in Figure~\ref{fig:ablation-mazes}. We describe these channels as Infrastructure Channels in later experiments.
...
\section{Expanded ablation experiments}\label{sec:expanded_ablation_methodology}

To derive further insights from our earlier results, we ablate regions of the network based on their success in modifying model behaviour in the previous experiments. These targeted ablations address RQs 4, 5, and 6 by systematically testing which network components are necessary for entity-specific targeting, why steering effectiveness varies, and how functionality is distributed across layers. Our experiment uses a modified maze which removes all walls, and has the keys and gem randomly distributed in the corners of a square, shown on the right in Figure~\ref{fig:ablation-mazes}.

In these experiments, channel selection was entity-specific: when testing blue key collection, we used channels ranked by their blue key performance; for green key, channels ranked by green key performance, and so on. The specific ranking criterion varied by ablation type as described below.

We perform six targeted ablations based on our patching and navigation findings:

\textbf{Ablation 1 (Intervention spans):} We define \textit{intervention spans} as the activation value ranges within which our patching experiments successfully redirected the agent toward a target location. For this ablation, we zero out any activations that fall within these ranges. Specifically, for entity $e$ and channel $c$, we set $h_{c,i,j} = 0$ whenever the activation falls within the intervention span $\mathcal{S}_{e,c} = [v_{\min}, v_{\max}]$, where $v_{\min}$ and $v_{\max}$ are the minimum and maximum activation values that produced successful redirection in our patching experiments (Section~\ref{subsubsec:steering_interventions}).
...
\textbf{Ablation 3 (Infrastructure channel Ablation):} We ablate the top 10 infrastructure channels identified in Section~\ref{subsec:discovering_infrastructure} as most critical for a given entity (i.e., those whose individual removal most reduced collection rates), setting these channels to zero while leaving the rest unchanged.

\textbf{Ablation 4 (Combined):} We ablate both the navigation channels that had the best collection rates for a given entity and the intervention spans for a given entity simultaneously.

\textbf{Ablation 5 (Inverse):} We preserve only the intervention spans that successfully redirected the agent in the presence of entity $e$, ablating everything else. This tests whether these spans alone are sufficient for navigation.
...
All probes were linear classifiers trained with the Adam optimizer (learning rate 0.01) for 200 epochs, which we found sufficient for convergence across all probe configurations.
...
This chapter detailed the methodologies employed to investigate goal representation and control in a deep reinforcement learning agent. It began with a description of the Procgen Heist environment and the model architecture used. Key interpretability techniques were then outlined, including feature visualisation, comparative activation analysis, SAE training, activation patching interventions, and systematic ablation studies. Finally, probing experiments were described to localise goal and navigation information across network layers, providing a framework for understanding the results discussed in the subsequent chapter.
