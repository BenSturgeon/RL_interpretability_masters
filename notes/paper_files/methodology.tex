...
Hyperparameter selection followed conventions established in prior work on CNN SAEs~\citep{gorton_2024_missing}. Expansion factors were kept modest (2--4$\times$) because the base model's convolutional layers already have relatively few channels (16--64), and excessive expansion can lead to feature splitting without interpretability gains. The smaller expansion factor for conv1a reflects its role in learning low-level features that require less disentanglement. L1 coefficients were tuned per-layer to balance reconstruction fidelity against sparsity: early layers (conv1a) used smaller coefficients (1e-4) to preserve fine-grained spatial information, while deeper layers (conv4a) tolerated larger coefficients (5e-3) as their representations are more abstract and naturally sparser.
