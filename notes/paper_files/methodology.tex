...
\label{sec:interp_techniques}
    ...
    The method involved placing a specific entity into an artificially generated environment using a box maze with the shape visible in figure\ref{fig:intervention-experiment}. This design was chosen because it was challenging enough that the agent would need to retain its navigational abilities to reach either target, while providing a clear decision point where the agent needed to choose between pursuing the original entity or our artificial target zone.
    
    The intervention involves masking the channel to 0 and setting a target region of the channel to a specified value. In this experiment, we would do a parameter sweep over a range of values from 0 to the maximum observed value that we observed the channel producing over 10000 sample inputs. By running this sweep, we can get a sense of what the different channels do at different intervention strengths. We kept the length of the episodes short so that the agent would either be able to reach the target zone, or the actual entity, but not both to keep the results clear.


    We scaled up this approach by running a static intervention across every channel in the SAE, and then measured how often it would deviate to the area we targeted. For each entity-channel combination, we ran 100 trials with the activation value incrementing according to: activation value = trial number × (max range / number of trials). This allowed us to systematically identify whether channels would only influence navigation when activated within specific value ranges.
