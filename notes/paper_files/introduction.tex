...
Our investigation seeks to build on the work of\cite{mini_2023_understanding}, who found key parts of the model that could be used to control and steer behavior. By exploring how models trained in the Procgen Heist environment\citep{cobbe_2020_leveraging} approach multiobjective maze solving, we aim to understand how neural networks represent and pursue multiple sequential goals.

% Combined Thesis Introduction
This thesis explores the mechanistic interpretability of reinforcement learning agents, specifically focusing on how these agents represent and pursue goals in complex, multi-objective environments. Building upon work that gives us tools for understanding neural networks through intervening on their operations via activation steering, as well as unsupervised methods to disentangle neural network representations such as Sparse AutoEncoders (SAEs), this research employs and extends these techniques by applying them to a CNN-based reinforcement learning agent trained on the Procgen Heist environment. The primary aim is to uncover the internal mechanisms by which the agent navigates, identifies targets, and prioritizes objectives, with a view to developing more generalizable insights into the workings of deep RL models.
